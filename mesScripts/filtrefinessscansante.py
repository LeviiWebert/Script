...
FALLBACK_COL      = "finessj"     # ← le nom de la colonne dans dfA qui contient le code FINESS supposé
...
def try_match_on_column(candidates: pd.DataFrame, column: str, tokens_req: list, debug_prefix="") -> list:
    ...
    # Étape 'ANY'
    if DEBUG:
        debug_print(f"{debug_prefix}--- ANY sur '{column}' ---")
    for idxB, rowB in candidates.iterrows():
        nom_b = rowB.get(column, "")
        tokensB = tokenize_df(nom_b)
...
def match_row(rowA, dfB_indexed, idxA=None):
    ...
    Si on obtient 0 code ou plusieurs codes, on essaye un fallback sur la colonne 'finessj' de dfA.
    Renvoie (code_finess, statut).  
    """
    ...
    def do_token_match(cand_df, tokens_req, label, debug_pref=""):
        ...
        Essaie sur cand_df (déjà filtré sur dépt/ville) :
          1) ALL sur 'Nom'
          2) Si aucun 'ALL', ANY sur 'Nom'
          3) Si > 1 ANY et Nom2 existe, désambiguïsation via 'ALL sur Nom2' puis 'ANY sur Nom2'
        ...
        # Si >1 ANY et Nom2 existe, on désambiguïse
    ...
    if not candidats_dept.empty and tokensA:
        ...
        if DEBUG:
            if not candidats_both.empty:
                ...
                debug_print(f"[Ligne {idxA}] Candidats après filtre DÉPT+VILLE nom ('{deptA}','{cityA}') → Noms disponibles = {noms_both}")
        ...
        if not candidats_both.empty:
            ...
            if codes_both:
                if len(codes_both) == 1:
                    ...
                    # Avant de renvoyer "PLUSIEURS CAS", on teste le fallback (colonne finessj)
                    fcol = rowA.get(FALLBACK_COL, "")
                    if pd.notna(fcol) and fcol.strip():
                        fcol = str(fcol).strip()
                        # Si fcol apparait dans codes_both, on renvoie fcol comme seul match
                        if fcol in codes_both:
                            if DEBUG:
                                debug_print(f"{prefix}  → Fallback finessj '{fcol}' trouvé dans codes_both")
                            return fcol, "1 - réussi (fallback finessj)"
    ...
    if not candidats_dept.empty and tokensA:
        ...
        if codes_dept:
            if len(codes_dept) == 1:
                ...
                # Fallback s'il existe une colonne finessj
                fcol = rowA.get(FALLBACK_COL, "")
                if pd.notna(fcol) and fcol.strip():
                    fcol = str(fcol).strip()
                    if fcol in codes_dept:
                        if DEBUG:
                            debug_print(f"{prefix}  → Fallback finessj '{fcol}' trouvé dans codes_dept")
                        return fcol, "1 - réussi (fallback finessj)"
                return "PLUSIEURS CAS", "PLUSIEURS CAS"
        else:
            if DEBUG:
    ...
    if not candidats_city.empty and tokensA:
        ...
        if codes_city:
            if len(codes_city) == 1:
                ...
                # Fallback avec finessj
                fcol = rowA.get(FALLBACK_COL, "")
                if pd.notna(fcol) and fcol.strip():
                    fcol = str(fcol).strip()
                    if fcol in codes_city:
                        if DEBUG:
                            debug_print(f"{prefix}  → Fallback finessj '{fcol}' trouvé dans codes_city")
                        return fcol, "1 - réussi (fallback finessj)"
                return "PLUSIEURS CAS", "PLUSIEURS CAS"
        else:
            if DEBUG:
    ...
    # ――― Aucune correspondance ou passage fallback finessj ――――――――――――――――――――
    fcol = rowA.get(FALLBACK_COL, "")
    if pd.notna(fcol) and fcol.strip():
        fcol = str(fcol).strip()
        # On peut essayer de voir si ce fcol existe **au moins** dans n'importe quel sous‐ensemble DF (dépt ou ville).
        # Méthode simple : vérifier dans dfB_indexed[deptA] et/ou dans tous dfB_indexed[] s’il y figure.
        # Ici on fait le test dans l'ensemble des candidats du département (puisque tous les DF sont indexés par dept):
        if fcol in dfB_indexed.get(deptA, pd.DataFrame())[COLB_CODE_FINESS].astype(str).tolist():
            if DEBUG:
                debug_print(f"{prefix}  → Fallback finessj '{fcol}' présent quelque part dans département {deptA}")
            return fcol, "1 - réussi (fallback finessj)"
...
def main():
    ...
    # On suppose que la colonne fallback existe (même si certaines lignes peuvent être vides)
    if FALLBACK_COL not in dfA.columns:
        # Si elle n'existe pas, on l'ajoute (vallue vide)
        dfA[FALLBACK_COL] = ""
    ...
    cols_to_keep = [nomA_col, COLA_DEPT, "City_norm_A", COLA_MOTS_SIG, FALLBACK_COL, "Code FINESS final", "Statut final"]
