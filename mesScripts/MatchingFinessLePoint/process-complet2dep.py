...
Spécificités intégrées :
  - Matching par département (deux premiers chiffres du code postal DF vs “Département” dans Data)
...
Usage :
  1. Modifier en début de script les chemins PATH_TABLE_A, PATH_TABLE_B, OUTPUT_PATH
  2. pip install pandas openpyxl
  3. python match_finess_optimise_dept.py
"""
...
STOPWORDS = {
    ...
    "GRAND","HÔPITAL","HOPITAL","CLINIQUE","MATERNITÉ","MATERNITE"
}
# Abréviations à conserver dans DF
...
def extract_significant(text: str) -> str:
    ...
    - token alphabétiques (> 3 lettres, hors STOPWORDS)
    """
    if pd.isna(text):
        return ""
...
def tokenize_df(text: str) -> list:
    ...
    - On garde tokens > 3 lettres hors STOPWORDS, et on conserve aussi les abréviations hospitalières (CH, CHU, CHI, HCL, GHL).
    """
    if pd.isna(text) or not str(text).strip():
        return []
    s = re.sub(r"[’'\-–_/(),]", " ", str(text).upper())
...
def try_match_on_column(candidates: pd.DataFrame, column: str, tokens_req: list, debug_prefix="") -> list:
    """
    Parmi candidats (même département), renvoie d'abord la liste des Code FINESS
    où tous les tokens_req ∈ tokenize_df(rowB[column]). Si aucun, renvoie ceux où
    au moins un token_req ∈ tokenize_df(rowB[column]). Avec logs si DEBUG.
    """
    ...
    for idxB, rowB in candidates.iterrows():
        ...
        # Ne pas encore ajouter à matched_any
    ...
    for idxB, rowB in candidates.iterrows():
        ...
        if any(tok in tokensB for tok in tokens_req):
            ...
            if DEBUG:
                print(f"{debug_prefix}   → ANY match: au moins un des {tokens_req} est dans {tokensB}")

    return matched_any

def match_row(rowA, dfB_indexed, idxA=None):
    """
    Pour une ligne de Data, retourne (code_finess, statut) selon :
    1) tokens_A = tokenize_data("Mots significatifs") ou fallback abrév si vide.
    2) Filtrer DF sur même département.
    3) Essayer matching sur "Nom" (tous tokens puis au moins un).
    4) Si plusieurs, retenter sur "Nom2". Si toujours plusieurs, "PLUSIEURS CAS".
    """
    prefix = f"[Data idx {idxA}] " if idxA is not None else ""

    deptA = str(rowA[COLA_DEPT]).zfill(2)  # deux chiffres ex. '01'
    ...
    if not tokensA:
        ...
        if DEBUG:
            print(f"{prefix}Fallback abréviations sur '{source}' → tokensA = {tokensA}")

    candidats = dfB_indexed.get(deptA)
    if candidats is None or candidats.empty:
        if DEBUG:
            print(f"{prefix}Aucun candidat DF pour le département '{deptA}'")
        return "", "0 - pas bon"

    if DEBUG:
        print(f"{prefix}Nombre de candidats pour département '{deptA}': {len(candidats)}")

    # 1) Essayer matching sur "Nom"
    if DEBUG:
...
def main():
    ...
    # Déterminer colonne Nom_hopital ou Nom_clinique
    ...
    for col in (COLA_DEPT, COLA_MOTS_SIG):
    ...
    # 2) Normaliser "Département" dans Data : garder 2 chiffres
    dfA[COLA_DEPT] = dfA[COLA_DEPT].astype(str).str.strip().str.zfill(2)
    if DEBUG:
        print("==> Colonne 'Département' formatée sur 2 chiffres dans Data")

    # 3) Préparer DF : extraire code département (2 premiers chars de "Ville")
    dfB["Dept"] = dfB[COLB_VILLE].astype(str).str.strip().str[:2].str.zfill(2)
    if DEBUG:
        print("==> Colonne 'Dept' créée dans DF (2 premiers chiffres de 'Ville')")
    ...
    # 5) Préparer et enregistrer le résultat
    cols_to_keep = [nomA_col, COLA_DEPT, COLA_MOTS_SIG, "Code FINESS final", "Statut final"]
