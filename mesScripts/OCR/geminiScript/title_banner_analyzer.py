...
try:
    from config_banner_analyzer import *
except ImportError:
    import os
    print("⚠️  Fichier config_banner_analyzer.py non trouvé, utilisation des valeurs par défaut")
...
class TitleBannerAnalyzer:
    def __init__(self, images_base_path, output_excel="titres_classements_analysis.xlsx"):
        """
        Analyseur de bannières de titres avec détection automatique
    ...
    def setup_gemini(self):
        ...
        try:
            ...
            print("💡 Vérifiez votre clé API dans config_banner_analyzer.py")
            self.model = None
    ...
    def find_image_pairs(self):
        ...
        Structure : img*.jpg (originales) + DOSSIER_*.jpg (tableaux découpés)
        ...
        print(f"🔍 {len(pairs)} paires d'images trouvées au total")
        return pairs
    ...
    def detect_hospital_type_with_gemini(self, table_image_path):
        ...
        try:
            ...
            Le titre est clairement indiquer dans le tabkeau en haut à gauche dans l'entete en gros caractère.
    ...
    def process_all_images(self):
        """
        Lance le traitement complet de toutes les paires d'images
        """
        print("🚀 Début de l'analyse des bannières de titres...")
        
        # Trouver toutes les paires d'images
        image_pairs = self.find_image_pairs()
        
        if not image_pairs:
            print("❌ Aucune paire d'images trouvée")
            return
...
def main():
    """Fonction principale"""
    # Configuration depuis le fichier config
    OUTPUT_FILE = f"titres_classements_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    ...
    if not os.path.exists(IMAGES_BASE_PATH):
        ...
        print("💡 Modifiez la variable IMAGES_BASE_PATH dans config_banner_analyzer.py")
        return
    
    try:
        # Création et lancement de l'analyseur
        analyzer = TitleBannerAnalyzer(
